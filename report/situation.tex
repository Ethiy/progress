...
\begin{document}
    ...
    \begin{tikzpicture}
        ...
        \path (pc.west)+(-3,2) node (oi) [data, text width=2cm, fill={rgb,255:red,101;green,203;blue,94}] {Oriented Images};
        \path (pc.east)+(3.5,0) node (sr) [data, text width=2.5cm, fill={rgb,255:red,56;green,88;blue,140}, text=wheat] {Primitives, Meshes};
        \path (sr.east)+(3,0) node (um) [data, text width=2cm, fill={rgb,255:red,69;green,50;blue,126}, text=wheat] {3D Urban Models};
        ...
        \path (um.north)+(0,5) node (human) [sensor] {Human Operator};

        \path[draw, ->] (camera.east) -- node [above] {} (oi.west);
        \path[draw, ->] (oi.355) -| ($(oi.355) ! .5 ! (pc.165)$) |- (pc.165);
        \path[draw, ->, red, dash dot] (human.165) -| ($(human.165) ! .7 ! ($(oi.355) ! .5 ! (pc.165)$)$) |- ($(oi.355) ! .5 ! (pc.165)$);
        \path[draw, ->] (oi.5) -| ($(oi.5) ! .8 ! (sr.170)$) |- (sr.170);
        \path[draw, ->, red, dash dot] (human.180) -| ($(human.180) ! .7 ! ($(oi.5) ! .5 ! (sr.170)$)$) |- ($(oi.5) ! .8 ! (sr.170)$);
        \path[draw, ->] (lidar.east) -- node [above] {} (pc.west);
        \path[draw, ->] (pc.east) -- node [above] {} (sr.west);
        \path[draw, ->] (sr.east) -- node [above] {} (um.west);
        \path[draw, ->, red, dash dot] (human.190) -| ($(sr) ! .5 ! (um)$);
        \path[draw, ->] (um.east) -- node [above] {} (om.west);
