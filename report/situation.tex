...
\begin{document}
    ...
    \begin{tikzpicture}
        ...
        \path[draw, ->] (camera.east) -- node [above] {Aquired Signal} (oi.west);
        \path[draw, ->] (oi.355) -| ($(oi.355) ! .5 ! (pc.165)$) node [left, text width=2.5em, align=center] {Photogrammetry} |- (pc.165);
        \path[draw, ->, red, dash dot] (human.165) -| node [above right] {Feature points intake} ($(human.165) ! .7 ! ($(oi.355) ! .5 ! (pc.165)$)$) |- ($(oi.355) ! .5 ! (pc.165)$);
        \path[draw, ->] (oi.5) node [above right, text width=6em, align=center] {Primitive extraction} -| 
         ($(oi.5) ! .8 ! (sr.170)$) |- (sr.170);
        ...
        \path[draw, ->] (lidar.east) -- node [above] {Acquired Signal} (pc.west);
        ...
        \path[draw, ->] (um.east) -- node [below, text width=7em, align=center] {Post-processing \& Corrections} (om.west);
